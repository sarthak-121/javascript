...
class Input {
    ...
    constructor(tiles) {
        ...
        canvas.addEventListener('mousedown', event => {
            ...
            console.log('--119--event.which--', event.which);
    ...
    rightClickHandler() {
        for(let i = 0 ; i < 9 ; i++) {
            for(let j = 0 ; j < 9 ; j++) {
                if(tiles[i][j].clicked(this.cx, this.cy)) {
                    if(tiles[i][j].bomb) {
...
function time(clock) {
    ...
    ctx.fillStyle = "red";
    ctx.textAlign = "center";
    ctx.fillText(clock.toString(), 50, 450);
    //ctx.drawImage(document.getElementById("flag_img"))
...
let timeAndFlagTimeout = setTimeout(time, 1, clock);
let gameloopTimeout = setTimeout(gameloop, 33);
